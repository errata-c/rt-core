...
public:
	VertexArray() {
		glCreateVertexArrays(1, &id); checkError();
	}
	...
	VertexArray& operator=(VertexArray&& other) noexcept {
		if (isValid()) {
			glDeleteVertexArrays(1, &id); checkError();
		}
	...
	void bindVertex(const rt::BufferBase & buff, GLuint index, intptr_t offset, GLsizei stride) {
	...
	void bindIndex(const BufferBase& buffer) {
	...
	void bind() {
		glBindVertexArray(id); checkError();
	}
	void unbind() {
		glBindVertexArray(0); checkError();
	}
	...
	void attribDivisor(GLuint attrIndex, GLuint divisor) {
		glVertexArrayBindingDivisor(id, attrIndex, divisor); checkError();
	}
	void attribBinding(GLuint attrIndex, GLuint bindingIndex) {
		glVertexArrayAttribBinding(id, attrIndex, bindingIndex); checkError();
	}


	void attribFormatF32(GLuint attr, GLuint dimm, GLsizei offset, bool normalized = false, Type type = Type::Float) {
		glVertexArrayAttribFormat(id, attr, dimm, convertGL(type), normalized, offset); checkError();
	}
	void attribFormatF64(GLuint attr, GLuint dimm, GLsizei offset, Type type = Type::Float) {
		glVertexArrayAttribLFormat(id, attr, dimm, convertGL(type), offset); checkError();
	}
	void attribFormatI32(GLuint attr, GLuint dimm, GLsizei offset, Type type = Type::I32) {
		glVertexArrayAttribIFormat(id, attr, dimm, convertGL(type), offset); checkError();
	}
	...
	bool isAttribEnabled(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &val); checkError();
		return static_cast<bool>(val);
	}
	bool isAttribF64(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_LONG, &val); checkError();
		return static_cast<bool>(val);
	}
	bool isAttribNormalized(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, &val); checkError();
		return static_cast<bool>(val);
	}
	bool isAttribInteger(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_INTEGER, &val); checkError();
		return static_cast<bool>(val);
	}
	int getAttribDim(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_SIZE, &val); checkError();
		return val;
	}
	int getAttribStride(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_STRIDE, &val); checkError();
		return val;
	}
	int getAttribDivisor(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_DIVISOR, &val); checkError();
		return val;
	}
	GLenum getAttribType(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_TYPE, &val); checkError();
		return static_cast<GLenum>(val);
	}
	int getAttribRelativeOffset(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_RELATIVE_OFFSET, &val); checkError();
		return static_cast<int>(val);
	}
	int64_t getAttribOffset(GLuint attr) const {
		assert(isValid());
		GLint64 val;
		glGetVertexArrayIndexed64iv(id, attr, GL_VERTEX_BINDING_OFFSET, &val); checkError();
		return val;
	}
	GLuint getAttribVertexBufferId(GLuint attr) const {
		assert(isValid());
		GLint val;
		glGetVertexArrayIndexediv(id, attr, GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, &val); checkError();
	...
	GLuint getIndexBufferId() const {
		assert(isValid());
		int val;
		glGetVertexArrayiv(id, GL_ELEMENT_ARRAY_BUFFER_BINDING, &val); checkError();
		return static_cast<GLuint>(val);
	}
